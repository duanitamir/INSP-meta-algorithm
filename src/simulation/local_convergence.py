...
class LocalConvergence:
    ...
    def __init__(self, node_id: int, graph: LocalGraph, state: NodeState,
                 send_message: Callable[[Message], None], threshold: float,
                 quorum_threshold: float) -> None:
    ...
    def gossip(self, round_number: int) -> None:
        ...
        payload = {"type": "CONVERGENCE_VOTE", "vote": self.vote if self.vote is not None else False,
                   "should_stop": self.vote if self.vote is not None else False, "round": round_number,
                   "weight": weight, "active": self.state.get("active", True),
                   "matched": self.state.is_matched()}
        for neighbor_id in random.sample(neighbors, min(3, len(neighbors))):
            self._send_message(Message(sender=self.node_id, recipient=neighbor_id,
                                       payload=payload.copy(), round_num=round_number))

    def should_stop(self) -> bool:
        if self.state.get(NodeState.TENTATIVE_PARTNER) is not None or not self.known_votes:
            return False
        return (sum(1 for vote in self.known_votes.values() if vote) / len(self.known_votes)
